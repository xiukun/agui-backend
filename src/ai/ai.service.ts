...
export class AiService {
    ...
    constructor(
        ...
        this.agent = createAgent({
            ...
            tools: [this.webSearchTool, this.sendMailTool, this.cronJobTool, this.timeNowTool],
...
调用规则：需要最新信息时用 web_search；需要发送邮件时用 send_mail；需要定时执行任务时用 cron_job（add）；需要知道当前时间时用 time_now。仅当用户明确要求查看/创建/切换定时任务时才操作 cron_job。`,
        })
    }
