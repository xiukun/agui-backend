...
export class JobAgentService {
    ...
    async runJob(instruction: string, options?: { jobId?: string }): Promise<string> {
        ...
        try {
            ...
            this.logger.warn(`Failed to build memory context: ${(e as Error).message}`);
        }
        ...
        try {
            ...
            if (!toolCalls.length) {
                ...
                try {
                    ...
                    this.logger.warn(`Failed to write job_execution memory: ${(e as Error).message}`);
                }
                return final;
            }
            ...
            try {
                await this.memoryService.upsertMemory({
                    scope: 'job_execution',
                    ownerId: jobId ?? 'global',
                    content: `job_instruction:\n${instruction.slice(0, 2000)}\n\nexecution_error:\n${String((e as Error)?.message ?? e).slice(
                        0,
                        4000,
                    )}`,
